...
test("injects only the latest active Todo snapshot into model context", () => {
  const base = [{ role: "user", content: "continue", timestamp: 1 }];
  const state = {
    nextId: 4,
    tasks: [
      { id: 1, subject: "Finished setup", status: "completed" },
      { id: 2, subject: "Implement bridge", status: "in_progress", activeForm: "writing bridge" },
      { id: 3, subject: "Verify bridge", status: "pending", blockedBy: [2] },
    ],
  };

  const first = injectTodoContext(base, state);
  assert.ok(first);
  assert.equal(first.messages.length, 2);
  const snapshot = first.messages.at(-1);
  assert.equal(snapshot.role, "custom");
  assert.equal(snapshot.customType, "pi-maestro-todo-context");
  assert.equal(snapshot.display, false);
  assert.match(snapshot.content, /#2 \[in_progress\] Implement bridge; activeForm=writing bridge/);
  assert.match(snapshot.content, /#3 \[blocked\] Verify bridge; blockedBy=#2/);
  assert.doesNotMatch(snapshot.content, /Finished setup/);

  const replaced = injectTodoContext(first.messages, {
    nextId: 4,
    tasks: [{ id: 3, subject: "Verify bridge", status: "in_progress" }],
  });
  assert.ok(replaced);
  assert.equal(replaced.messages.filter((message) => message.role === "custom" && message.customType === "pi-maestro-todo-context").length, 1);
  assert.match(replaced.messages.at(-1).content, /#3 \[in_progress\] Verify bridge/);
  assert.doesNotMatch(replaced.messages.at(-1).content, /Implement bridge/);

  const cleared = injectTodoContext(replaced.messages, {
    nextId: 4,
    tasks: [{ id: 3, subject: "Verify bridge", status: "completed" }],
  });
  assert.ok(cleared);
...
test("keeps the Todo center inside narrow widths and preserves task priority", async () => {
  const { TodoCenter } = await import("../src/center.ts");
  const tasks = [
