...
export const TODO_CONTEXT_TYPE = "pi-maestro-todo-context";

export function injectTodoContext(messages: AgentMessage[], state: TaskState): { messages: AgentMessage[] } | undefined {
  const withoutPrevious = messages.filter((message) => !(message.role === "custom" && message.customType === TODO_CONTEXT_TYPE));
  const content = activeTodoContext(state);
  if (!content) return withoutPrevious.length === messages.length ? undefined : { messages: withoutPrevious };
  ...
  const snapshot: AgentMessage = {
    ...
    content,
    display: false,
    details: { source: "pi-maestro-todo", taskCount: state.tasks.length },
    timestamp: Date.now(),
  };
  return { messages: [...withoutPrevious, snapshot] };
}
...
function registerCommands(pi: ExtensionAPI): void {
  pi.registerCommand(COMMAND_NAME, {
    description: "Show and filter the current Todo list",
    handler: async (_args, ctx) => {
      await openTodoCenter(ctx);
    },
  });
  pi.registerCommand("maestro-todo", {
    description: "Open the Maestro-style Todo center",
    handler: async (_args, ctx) => {
      await openTodoCenter(ctx);
    },
  });
...
export default function register(pi: ExtensionAPI): void {
  ...
  pi.on("context", (event, ctx) => {
    return injectTodoContext(event.messages, stateFor(sessionId(ctx)));
  });
