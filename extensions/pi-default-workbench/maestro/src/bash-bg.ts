...
const BashBgParams = Type.Object({
  ...
  cwd: Type.Optional(Type.String({ description: "Working directory for run and start; relative paths resolve against the session working directory" })),
...
export function registerBashBg(pi: ExtensionAPI): void {
  ...
  const cleanupCompletedJobs = (): void => {
    const now = Date.now();
    const completed = [...jobs.values()]
      .filter((job) => job.done)
      .sort((a, b) => (a.finishedAt ?? 0) - (b.finishedAt ?? 0));
    const remove = completed.filter((job, index) =>
      now - (job.finishedAt ?? now) >= COMPLETED_JOB_TTL_MS || index < Math.max(0, completed.length - MAX_COMPLETED_JOBS),
    );
  ...
  cleanupTimer = setInterval(cleanupCompletedJobs, CLEANUP_INTERVAL_MS);
  cleanupTimer.unref();
  ...
  pi.registerTool({
    ...
    promptSnippet: "Use bash_bg only for unbounded or long-running commands.",
    promptGuidelines: [
