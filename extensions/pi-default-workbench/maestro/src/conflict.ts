...
async function resolveConflicts(cwd: string, targets: FlatConflict[], content: string, signal?: AbortSignal): Promise<string[]> {
  ...
  const written: Array<(typeof prepared)[number]> = [];
  try {
    ...
    for (const file of [...written].reverse()) {
      try {
        await writeFile(file.absolutePath, file.text, "utf8");
      } catch (rollbackError) {
        rollbackErrors.push(`${file.path}: ${rollbackError instanceof Error ? rollbackError.message : String(rollbackError)}`);
      }
    }
    const original = error instanceof Error ? error.message : String(error);
    if (rollbackErrors.length) throw new Error(`${original}; rollback failed: ${rollbackErrors.join("; ")}`);
    throw new Error(`${original}; all modified files were restored`);
  }
}
  ...
  pi.registerTool({
    ...
    async execute(
      ...
      if (params.action === "list") {
        ...
        return { content: [{ type: "text", text: listText(files) }], details: { conflicts: flatten(files).length } } as AgentToolResult<unknown>;
      }
