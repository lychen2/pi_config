...
import {
  ...
  type JsonObject,
  type LargeModeState,
} from "./large-mode-core.ts";

const CONTROLLER_PATH = fileURLToPath(import.meta.url);
const FLOW_LATEST_SOURCE = "npm:pi-maestro-flow@latest";
...
async function installedFlowSource(profile: FlowProfile): Promise<string> {
  const manifest = await readJsonObject(join(profile.npm, "node_modules", "pi-maestro-flow", "package.json"));
  if (manifest.name !== "pi-maestro-flow" || typeof manifest.version !== "string") {
    throw new Error("npm install did not produce a valid pi-maestro-flow manifest");
  }
  return flowSourceForVersion(manifest.version);
}

async function buildStagingProfile(flowSource: string, progress?: ProgressReporter): Promise<BuiltFlowProfile> {
  ...
  try {
    ...
    await mkdir(profile.npm, { recursive: true });
    ...
    const version = flowVersionFromSource(flowSource);
    const installSpec = `pi-maestro-flow@${version ?? "latest"}`;
    try {
    ...
    const installedSource = await installedFlowSource(profile);
    await runOfficialFlowSetup(profile, stagingHome, progress);
    progress?.(75, "安装完成，整理配置");

    await installBeautifyPackage(profile, installedSource, progress);
    ...
    await writeMarker(profile.npm, installedSource);
    await writeMarker(profile.maestro, installedSource);
    await rm(stagingHome, { recursive: true, force: true });
    progress?.(85, "验证完整 Flow profile");
    await validateProfile(profile, installedSource);
    return { profile, flowSource: installedSource };
  } catch (error) {
...
async function ensureInactiveVersion(
  flowSource: string,
  progress?: ProgressReporter,
  forceRebuild = false,
  ...
  const inactive = inactiveProfile();
  const targetSource = expectedSource
    ?? (flowSource === FLOW_LATEST_SOURCE ? flowSourceForVersion(await latestFlowVersion()) : flowSource);
  progress?.(20, "检查本地 Flow 缓存");
  const cached = await readMarker(inactive.npm);
  if (!forceRebuild && cached?.profileVersion === LARGE_PROFILE_VERSION && cached.flowSource === targetSource) {
    try {
      await validateProfile(inactive, cached.flowSource);
      progress?.(85, "缓存验证完成");
      return cached.flowSource;
    } catch (error) {
      progress?.(22, "本地 Flow 缓存已过期，重新构建");
    }
  }
  const built = await buildStagingProfile(flowSource, progress);
  await replaceInactiveProfile(built.profile);
  return built.flowSource;
}
...
async function activate(ctx: ExtensionCommandContext, progress: ProgressReporter): Promise<boolean> {
  ...
  let flowSource = FLOW_LATEST_SOURCE;
  flowSource = await ensureInactiveVersion(flowSource, progress);
...
async function updateFlow(apply: boolean, ctx: ExtensionCommandContext, progress: ProgressReporter): Promise<void> {
  ...
  const nextSource = FLOW_LATEST_SOURCE;
  if (current === latest) {
    const resolvedSource = await ensureInactiveVersion(nextSource, progress, false, flowSourceForVersion(latest));
    const repaired = stableState("default", resolvedSource, state, { reloadPending: state.reloadPending });
    await writeState(repaired);
    ctx.ui.notify(`Flow 已是最新版本 ${latest}；完整缓存已验证`, "info");
    return;
  }

  const resolvedSource = await ensureInactiveVersion(nextSource, progress, true, flowSourceForVersion(latest));
  const updated = stableState("default", resolvedSource, state, { reloadPending: state.reloadPending });
