...
type CallPatch = {
  original: GetCallRenderer;
  patched: GetCallRenderer;
  owners: Set<symbol>;
};
...
const CALL_PATCH = Symbol.for("pi.toolRails.callRendererPatch");
...
function renderAftMutationResult(
  ...
  const rawDiff = typeof details.diff === "string" ? details.diff : "";
  const diffAdditions = rawDiff
    ? rawDiff.split("\n").filter((line) => line.startsWith("+") && !line.startsWith("+++")).length
    : undefined;
  const diffDeletions = rawDiff
    ? rawDiff.split("\n").filter((line) => line.startsWith("-") && !line.startsWith("---")).length
    : undefined;
...
function releaseCall(
  ...
  patch.owners.delete(owner);
  if (patch.owners.size > 0) return;
  if (prototype.getCallRenderer === patch.patched) {
    prototype.getCallRenderer = patch.original;
  }
  if (shared[CALL_PATCH] === patch) delete shared[CALL_PATCH];
}

function renderCompactCall(name: string, args: unknown, theme: Theme, context: ResultContext): Text {
  const recordArgs = record(args);
  const rawPath = typeof recordArgs.path === "string" ? recordArgs.path
    : typeof recordArgs.file_path === "string" ? recordArgs.file_path
    : sourcePathFromContext(context);
  const label = theme.fg("accent", name);
  return reusableText(context, rawPath ? `${label} ${theme.fg("muted", rawPath)}` : label);
}

function installCallBridge(): () => void {
  const shared = globalThis as typeof globalThis & Record<symbol, unknown>;
  const prototype = ToolExecutionComponent.prototype as unknown as RendererPrototype;
  const owner = Symbol("pi.toolRails.callRenderer");
  const existing = shared[CALL_PATCH] as Partial<CallPatch> | undefined;
  if (existing?.original && existing.patched && existing.owners) {
    const patch = existing as CallPatch;
    patch.owners.add(owner);
    return () => releaseCall(shared, prototype, patch, owner);
  }
  if (typeof prototype.getCallRenderer !== "function") return () => {};

  const original = prototype.getCallRenderer;
  const patched: GetCallRenderer = function (): CallRenderer | undefined {
    const nativeCallRenderer = original.call(this);
    const name = (this as unknown as { toolName?: string }).toolName;
    if (name === "edit") {
      return (args, theme, context) => renderCompactCall(name, args, theme, context);
    }
    return nativeCallRenderer;
  };
  const patch: CallPatch = { original, patched, owners: new Set<symbol>([owner]) };
  shared[CALL_PATCH] = patch;
  prototype.getCallRenderer = patched;
  return () => releaseCall(shared, prototype, patch, owner);
}
...
export default function resultBridge(pi: ExtensionAPI): void {
  ...
  const cleanupCall = installCallBridge();
