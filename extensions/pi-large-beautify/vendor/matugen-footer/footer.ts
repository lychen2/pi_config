...
function styleContextSegment(
	...
	// The reference gauge used fixed macaron RGB. Matugen owns the complete
	// gauge, including its cells, so keep one semantic context color throughout.
	return renderStyleForSource(theme, colorSource, contextColor, label);
}
	...
	ctx.ui.setFooter((tui, theme, footerData) => {
		...
		return {
			...
			render(width: number): string[] {
				...
				const cwdLabel = renderStyleForSource(theme, colorSource, config.colors.cwd, cwdPlain);
