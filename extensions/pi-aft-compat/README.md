...
- Disables `aft_search` outside a Git worktree. AFT `grep`, FFF `ffgrep`, and FFF `fffind` remain available. Set `PI_AFT_SEARCH_ALLOW_NO_GIT=1` to opt in for a no-Git workspace.
