...
\begin{frame}{Implications and Impact}
  ...
  % Keep this block only when a limitation needs a dedicated explanation.
  % Otherwise qualify the affected result where it appears.
  \begin{block}{Conditions on interpretation (if needed)}
    \begin{itemize}
      \item State a concrete study/model condition and its consequence for this claim
      \item Example: One-site recruitment limits transfer to other settings
    \end{itemize}
  \end{block}
\end{frame}
