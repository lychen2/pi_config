...
% Keep this optional slide when these conditions materially affect interpretation.
% Replace examples with verified study facts; remove conditions that do not apply.
\begin{frame}{Interpretation Boundaries and Future Directions}
  \begin{enumerate}
    \item \textbf{Sample}: Recruitment from [population/site] leaves transfer to [other setting] untested.
    \item \textbf{Measures}: Self-report on [measure] may share method variance with [outcome].
    \item \textbf{Follow-up}: Observation through [time point] does not establish longer-term persistence.
    \item \textbf{Mechanism}: The tested model does not distinguish [alternative pathway].
  \end{enumerate}
\end{frame}
