...
% Retain a separate strengths/limitations section when required or useful.
% State concrete study conditions and their consequences for interpretation;
% otherwise qualify the affected result in the relevant discussion paragraph.
...
\textbf{Limitations:}
\begin{itemize}
    \item State a concrete study condition and its consequence for interpretation
    \item Example: Cross-sectional measurement does not establish temporal order
    \item Example: Recruitment from [specific population] leaves transfer to other populations untested
\end{itemize}
