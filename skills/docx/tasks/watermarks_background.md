...
1. Audit the document:
   ```bash
   python $HOME/.pi/agent/skills/docx/scripts/watermark_audit_remove.py input.docx --mode report
   ```

2. Remove (heuristic) by matching a substring inside the watermark text:
   ```bash
   python $HOME/.pi/agent/skills/docx/scripts/watermark_audit_remove.py input.docx --mode remove --contains DRAFT --out cleaned.docx
   ```

3. Render and diff (recommended for QA/regressions):
   ```bash
   python $HOME/.pi/agent/skills/docx/scripts/render_and_diff.py input.docx cleaned.docx --outdir diff_watermark
   ```
