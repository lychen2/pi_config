...
1. **Inspect** how many revisions exist:
   ```bash
   python $HOME/.pi/agent/skills/docx/scripts/accept_tracked_changes.py input.docx --mode report
   ```
2. **Accept** all tracked changes into a clean copy:
   ...
   (Or reject):
   ```bash
   python $HOME/.pi/agent/skills/docx/scripts/accept_tracked_changes.py input.docx --mode reject --out rejected.docx
   ```
3. **Render → PNG review** (required):
   ```bash
   python $HOME/.pi/agent/skills/docx/render_docx.py accepted.docx --output_dir out_accept
   ```
