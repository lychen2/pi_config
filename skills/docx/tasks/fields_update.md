...
```bash
python $HOME/.pi/agent/skills/docx/scripts/fields_report.py ./input.docx
```
...
python $HOME/.pi/agent/skills/docx/scripts/flatten_ref_fields.py input.docx --out ref_flattened.docx

# Materialize SEQ/REF results (e.g., caption numbers / cross-refs)
python $HOME/.pi/agent/skills/docx/scripts/fields_materialize.py ref_flattened.docx --out fields_materialized.docx
```
