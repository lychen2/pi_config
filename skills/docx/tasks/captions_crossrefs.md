...
- `$HOME/.pi/agent/skills/docx/scripts/captions_and_crossrefs.py` — insert caption paragraphs + optional bookmarks around the caption number
- `$HOME/.pi/agent/skills/docx/scripts/insert_ref_fields.py` — replace `[[REF:bookmark]]` markers with real `REF` fields
- `$HOME/.pi/agent/skills/docx/scripts/fields_materialize.py` — materialize `SEQ/REF` *display text* so headless renders show the correct numbers
...
```bash
python $HOME/.pi/agent/skills/docx/scripts/captions_and_crossrefs.py \
  ./in.docx \
  ./with_captions.docx \
  --tables --figures \
...
```bash
python $HOME/.pi/agent/skills/docx/scripts/insert_ref_fields.py \
  ./with_captions.docx \
  ./with_refs.docx
```
...
```bash
python $HOME/.pi/agent/skills/docx/scripts/fields_materialize.py \
  ./with_refs.docx \
  --out ./with_refs_materialized.docx
```
...
```bash
python $HOME/.pi/agent/skills/docx/scripts/fields_materialize.py ./with_refs.docx --out ./out.docx --only REF
```

### 4) Render and visually QA
```bash
python $HOME/.pi/agent/skills/docx/render_docx.py ./with_refs_materialized.docx --output_dir ./out_caps
```
Inspect the PNGs.
