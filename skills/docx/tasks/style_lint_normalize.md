...
- `$HOME/.pi/agent/skills/docx/scripts/style_lint.py` — report likely inconsistencies
- `$HOME/.pi/agent/skills/docx/scripts/style_normalize.py` — conservative cleanup (optional)
...
## 1) Lint
```bash
python $HOME/.pi/agent/skills/docx/scripts/style_lint.py input.docx --json ./style_report.json
```
What to look for:
...
```bash
python $HOME/.pi/agent/skills/docx/scripts/style_normalize.py input.docx out_normalized.docx
```


> Tip: `style_normalize.py` also accepts `--out` as an alias:
> ```bash
> python $HOME/.pi/agent/skills/docx/scripts/style_normalize.py input.docx --out out_normalized.docx
> ```
...
```bash
python $HOME/.pi/agent/skills/docx/scripts/style_normalize.py input.docx out_normalized.docx --clear_paragraph_format
```

### C) Enforce a simple heading spacing rule
Useful when headings are visually inconsistent (space-after drift):
```bash
python $HOME/.pi/agent/skills/docx/scripts/style_normalize.py input.docx out_normalized.docx --enforce_heading_spacing
```

## Visual QA gate
```bash
python $HOME/.pi/agent/skills/docx/render_docx.py out_normalized.docx --output_dir ./out_norm
```
Success criteria:
