...
1. Apply the template parts to your doc:
   ```bash
   python $HOME/.pi/agent/skills/docx/scripts/apply_template_styles.py --template template.dotx --target report.docx --out report_styled.docx
   ```

2. Render and review:
   ```bash
   python $HOME/.pi/agent/skills/docx/render_docx.py report_styled.docx --output_dir out_styled
   ```
