...
This bundle provides `$HOME/.pi/agent/skills/docx/scripts/internal_nav.py` to add:
...
```bash
python $HOME/.pi/agent/skills/docx/scripts/captions_and_crossrefs.py ./in.docx ./with_caps.docx --figures --tables --bookmarks
```

3) Add navigation

```bash
python $HOME/.pi/agent/skills/docx/scripts/internal_nav.py ./with_caps.docx --out ./with_nav.docx
```

4) Render and verify

```bash
python $HOME/.pi/agent/skills/docx/render_docx.py ./with_nav.docx --output_dir ./out_nav
```
