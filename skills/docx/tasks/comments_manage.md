...
If the task is to *insert* new comments (not just extract/strip), use the OOXML-level guide: `ooxml/comments.md` (via `$HOME/.pi/agent/skills/docx/scripts/docx_ooxml_patch.py`).

## Add comments at scale (review mode)
For programmatic review injection (multiple comments across the document), use:
```bash
python $HOME/.pi/agent/skills/docx/scripts/comments_add.py input.docx --out reviewed.docx --author "Reviewer"   --add "Payment Terms=Please confirm Net 45 is acceptable."   --add "Governing Law=Prefer Delaware; any constraints?"   --ignore_case
```
Notes:
...
```bash
python $HOME/.pi/agent/skills/docx/scripts/comments_extract.py reviewed.docx --out comments.json
...
python $HOME/.pi/agent/skills/docx/scripts/comments_apply_patch.py reviewed.docx patch.json --out reviewed_v2.docx
```
...
```bash
python $HOME/.pi/agent/skills/docx/scripts/comments_extract.py input.docx --out comments.json
```
...
python $HOME/.pi/agent/skills/docx/scripts/comments_strip.py accepted.docx --out final_clean.docx
python $HOME/.pi/agent/skills/docx/render_docx.py final_clean.docx --output_dir out_final_clean
```
