...
python $HOME/.pi/agent/skills/docx/scripts/comments_strip.py input.docx --out no_comments.docx

# 3) Accept tracked changes (finalization)
python $HOME/.pi/agent/skills/docx/scripts/accept_tracked_changes.py input.docx --mode accept --out accepted.docx

# 4) Accessibility audit (+ optional safe fixes)
python $HOME/.pi/agent/skills/docx/scripts/a11y_audit.py input.docx
python $HOME/.pi/agent/skills/docx/scripts/a11y_audit.py input.docx --out_json a11y_report.json
python $HOME/.pi/agent/skills/docx/scripts/a11y_audit.py input.docx --fix_image_alt from_filename --out a11y_fixed.docx

# 5) Redact sensitive text (layout-preserving by default)
python $HOME/.pi/agent/skills/docx/scripts/redact_docx.py input.docx redacted.docx --emails --phones
```
...
- `$HOME/.pi/agent/skills/docx/scripts/docx_ooxml_patch.py` — low-level OOXML patch helper (tracked changes, comments, hyperlinks, relationships). Other scripts reuse this.
- `$HOME/.pi/agent/skills/docx/scripts/fields_materialize.py` — materialize `SEQ`/`REF` field *display text* for deterministic headless rendering/QA.
...
- `$HOME/.pi/agent/skills/docx/scripts/render_and_diff.py` — render + per-page image diff between two DOCXs.
- `$HOME/.pi/agent/skills/docx/scripts/content_controls.py` — list / wrap / fill Word content controls (SDTs) for forms/templates.
- `$HOME/.pi/agent/skills/docx/scripts/captions_and_crossrefs.py` — insert Caption paragraphs for tables/figures + optional bookmarks around caption numbers.
- `$HOME/.pi/agent/skills/docx/scripts/insert_ref_fields.py` — replace `[[REF:bookmark]]` markers with real `REF` fields (cross-references).
- `$HOME/.pi/agent/skills/docx/scripts/internal_nav.py` — add internal navigation links (static TOC + Top/Bottom + figN/tblN jump links).
- `$HOME/.pi/agent/skills/docx/scripts/style_lint.py` — report common formatting/style inconsistencies.
- `$HOME/.pi/agent/skills/docx/scripts/style_normalize.py` — conservative cleanup (clear run-level overrides; optional paragraph overrides).
- `$HOME/.pi/agent/skills/docx/scripts/redact_docx.py` — layout-preserving redaction/anonymization.
- `$HOME/.pi/agent/skills/docx/scripts/privacy_scrub.py` — remove personal metadata + `rsid*` attributes.
- `$HOME/.pi/agent/skills/docx/scripts/set_protection.py` — restrict editing (read-only / comments / forms).
- `$HOME/.pi/agent/skills/docx/scripts/comments_extract.py` — extract comments to JSON (text, author/date, resolved flag, anchored snippets).
- `$HOME/.pi/agent/skills/docx/scripts/comments_strip.py` — remove all comments (final-delivery mode).

**Audits / conversions / niche helpers:**
- `$HOME/.pi/agent/skills/docx/scripts/fields_report.py`, `$HOME/.pi/agent/skills/docx/scripts/heading_audit.py`, `$HOME/.pi/agent/skills/docx/scripts/section_audit.py`, `$HOME/.pi/agent/skills/docx/scripts/images_audit.py`, `$HOME/.pi/agent/skills/docx/scripts/footnotes_report.py`, `$HOME/.pi/agent/skills/docx/scripts/watermark_audit_remove.py`
- `$HOME/.pi/agent/skills/docx/scripts/xlsx_to_docx_table.py`, `$HOME/.pi/agent/skills/docx/scripts/docx_table_to_csv.py`
- `$HOME/.pi/agent/skills/docx/scripts/insert_toc.py`, `$HOME/.pi/agent/skills/docx/scripts/insert_note.py`, `$HOME/.pi/agent/skills/docx/scripts/apply_template_styles.py`, `$HOME/.pi/agent/skills/docx/scripts/accept_tracked_changes.py`, `$HOME/.pi/agent/skills/docx/scripts/make_fixtures.py`

**v7 additions (stress-test helpers):**
- `$HOME/.pi/agent/skills/docx/scripts/watermark_add.py` — add a detectable VML watermark object into an existing header.
- `$HOME/.pi/agent/skills/docx/scripts/comments_add.py` — add multiple comments (by paragraph substring match) and wire up comments.xml plumbing if needed.
- `$HOME/.pi/agent/skills/docx/scripts/comments_apply_patch.py` — append/replace comment text and mark/clear resolved state (`w:done=1`).
- `$HOME/.pi/agent/skills/docx/scripts/add_tracked_replacements.py` — generate tracked-change replacements (`<w:del>` + `<w:ins>`) in-place.
- `$HOME/.pi/agent/skills/docx/scripts/a11y_audit.py` — audit a11y issues; can also apply simple fixes via `--fix_table_headers` / `--fix_image_alt`.
- `$HOME/.pi/agent/skills/docx/scripts/flatten_ref_fields.py` — replace REF/PAGEREF field blocks with their cached visible text for deterministic rendering.

> `$HOME/.pi/agent/skills/docx/scripts/xlsx_to_docx_table.py` also marks header rows as repeating headers (`w:tblHeader`) to improve a11y and multi-page tables.
...
- `$HOME/.pi/agent/skills/docx/scripts/` — reusable helper scripts
